...
//	List<LearningReport> findByOrderByRemaAsc();
    ...
    @Query(value = "SELECT rema,user_id,created_at,start_time,end_time,course_id,remark,total_time,text_id,chapters_id FROM report WHERE report.user_id = user_id order by created_at desc",nativeQuery = true)
    List<LearningReport> findByuser_id(Integer user_id);
